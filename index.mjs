...
const xbw = createRequire(import.meta.url)("node-gyp-build")(__dirname);
const getIsoFilesFromArgs = async (args = []) => {
	const files = [];
	for (const arg of args.filter((a) => !a.startsWith("-"))) try {
		if ((await lstat(arg)).isDirectory()) for (const itemName of await readdir(arg)) try {
			const itemPath = join(arg, itemName);
			if ((await lstat(itemPath)).isDirectory()) files.push(...await getIsoFilesFromArgs([itemPath]));
...
const verifyWithAbgx360 = async (args = [], options, onProgress) => {
	if (Array.isArray(args)) {
		...
		return new Promise((resolve, reject) => {
			xbw.verifyWithAbgx360(abgxOptions.concat(files), (error, results, progress) => {
...
//#endregion
export { AbgxStatus, Region, getIsosInfo, verifyWithAbgx360 };
